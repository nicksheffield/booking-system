...
angular.module('app', ['ngResource', 'ngSanitize', 'satellizer', 'ui.bootstrap', 'ui.bootstrap.tpls', 'ui.router', 'ui.select', 'app.controllers', 'app.directives', 'app.resources', 'app.services', 'app.filters', 'app.routes', 'app.views', 'app.auth']);
'use strict';
...
angular.module('app.controllers').controller('issueCtrl', ["$scope", "$stateParams", "$store", "$location", "$http", "$invalidate", "$load", function ($scope, $stateParams, $store, $location, $http, $invalidate, $load) {
	...
	if (!$scope.booking) {
		...
		$scope.booking.$promise.then(function (p) {}, function (err) {
	...
	$scope.issue = function () {
		...
		var allChosen = $scope.booking.products.reduce(function (prev, cur) {
...
angular.module('app.controllers').controller('registerCtrl', ["$scope", "$auth", "$store", "$state", "$location", "User", function ($scope, $auth, $store, $state, $location, User) {
...
angular.module('app.controllers').controller('homeCtrl', ["$scope", "$rootScope", "$store", "$stateParams", "$prepare", "User", "Booking", function ($scope, $rootScope, $store, $stateParams, $prepare, User, Booking) {
	$scope.user = $store.user;
...
angular.module('app.controllers').controller('classTypeViewCtrl', ["$scope", "$stateParams", "$store", "$location", "$invalidate", function ($scope, $stateParams, $store, $location, $invalidate) {
...
angular.module('app.controllers').controller('classNewCtrl', ["$scope", "$stateParams", "$store", "$location", "$invalidate", "Group", function ($scope, $stateParams, $store, $location, $invalidate, Group) {
	...
	$scope.removeTutor = function (tutor) {
		$scope.selected.tutors = _.reject($scope.selected.tutors, function (t) {
			return t.id == tutor.id;
		});
...
angular.module('app.controllers').controller('classViewCtrl', ["$scope", "$stateParams", "$store", "$location", "$invalidate", function ($scope, $stateParams, $store, $location, $invalidate) {
...
angular.module('app.controllers').controller('productTypeViewCtrl', ["$scope", "$stateParams", "$store", "$invalidate", "$location", function ($scope, $stateParams, $store, $invalidate, $location) {
...
angular.module('app.controllers').controller('productViewCtrl', ["$scope", "$stateParams", "$state", "$store", "$location", "$invalidate", function ($scope, $stateParams, $state, $store, $location, $invalidate) {
	$scope.product = $store.get('products', $stateParams.id);
...
angular.module('app.controllers').controller('unitViewCtrl', ["$scope", "$stateParams", "$state", "$store", "$invalidate", "$location", function ($scope, $stateParams, $state, $store, $invalidate, $location) {
...
angular.module('app.controllers').controller('userEditCtrl', ["$scope", "$stateParams", "$state", "$store", "$location", "$invalidate", "User", function ($scope, $stateParams, $state, $store, $location, $invalidate, User) {
	...
	$scope.selected = {
		...
		role: _.find($scope.roles, function (r) {
...
angular.module('app.controllers').controller('userImportCtrl', ["$scope", "$q", "$invalidate", "$store", "$location", "$csv", "User", function ($scope, $q, $invalidate, $store, $location, $csv, User) {
	...
	$scope.loader.loaded = function (file) {
		if (file) {
			...
			cols = cols.map(function (col) {
				return fields[col] ? fields[col] : col;
			});
...
angular.module('app.controllers').controller('userViewCtrl', ["$scope", "$stateParams", "$store", "$invalidate", "$location", "Note", "User", function ($scope, $stateParams, $store, $invalidate, $location, Note, User) {
...
angular.module('app.controllers').controller('userViewNoteCtrl', ["$scope", "$stateParams", "$store", "$invalidate", "$location", "Note", function ($scope, $stateParams, $store, $invalidate, $location, Note) {
...
angular.module('app.directives').directive('sidebar', ["$store", "$auth", "$load", "$rootScope", "$http", function ($store, $auth, $load, $rootScope, $http) {
	function link(scope, el, attrs) {
		scope.auth = $auth;
		scope.user = $store.user;
		scope.store = $store;
...
angular.module('app.services').factory('$invalidate', ["$rootScope", "$auth", "$store", "$interval", "$load", function ($rootScope, $auth, $store, $interval, $load) {
	...
	service.all = function () {
		if ($auth.getPayload() && $auth.getPayload().admin) {
			...
			service.add(['user', 'users', 'groups', 'group_types', 'product_types', 'products', 'units', 'notes', 'booking_count']);
		} else {
			// if not logged in, or logged in as student
			service.add(['user', 'groups', 'products', 'units']);
		}
	};
...
angular.module('app.services').factory('$load', ["$prepare", "$auth", "$http", "User", "Group", "Group_Type", "Product_Type", "Product", "Unit", "Booking", "Note", function ($prepare, $auth, $http, User, Group, Group_Type, Product_Type, Product, Unit, Booking, Note) {
	var service = {};
	var events = [];
...
angular.module("app.views").run(["$templateCache", function ($templateCache) {
  $templateCache.put("components/home/home.html", "<page-header text=\"Welcome, {{ user.name }}\" breadcrumbs=\"false\"><btn type=\"primary\" url=\"home/edit_profile\" icon=\"pencil\">Edit Profile</btn></page-header><div class=\"page-content\"><div layout=\"col\"><div layout=\"row\" align=\"start sm-stretch\"><section ratio=\"1\"><h2>Your Current Bookings</h2><ul><li ng-repeat=\"booking in bookings\" ng-show=\"booking._priority < 3\"><a href=\"booking/{{ booking.id }}\">{{ booking.created_at | date }}</a></li></ul></section><section ratio=\"1\"><h2>Your Recent Bookings</h2><ul><li ng-repeat=\"booking in bookings\"><a href=\"booking/{{ booking.id }}\">{{ booking.created_at | date }}</a></li></ul></section><section ratio=\"1\"><h2>Your Previous Bookings</h2><ul><li ng-repeat=\"booking in bookings\" ng-show=\"booking._priority > 2\"><a href=\"booking/{{ booking.id }}\">{{ booking.created_at | date }}</a></li></ul></section></div></div></div>");
  ...
  $templateCache.put("directives/sidebar/sidebar.html", "<div class=\"sidebar\" layout=\"col sm-row\"><div class=\"site-logo\" layout=\"row\" justify=\"center\" align=\"center\"><!-- <img src=\"/images/logo2.svg\" alt=\"\" class=\"logo\" height=\"50\"> --><div class=\"logo\"><img src=\"/images/yoobee_logo_light.svg\" alt=\"\"><!-- <span class=\"caption\">Equipment Booking</span> --></div><!-- <h1>Booking System</h1> --></div><nav layout=\"col sm-row\"><!-- guest --><ul layout=\"col sm-row\" ng-if=\"!auth.isAuthenticated()\"><li class=\"title\"></li><li nav-link=\"Login\" url=\"/login\"></li><!-- <li nav-link=\"Register\" url=\"/register\"></li> --></ul><!-- student --><ul layout=\"col sm-row\" ng-if=\"auth.isAuthenticated() && user.admin == 0\"><li class=\"title\">Bookings</li><li nav-link=\"My Bookings\" url=\"/\"></li><li nav-link=\"Make a Booking\" url=\"/book\"></li><li class=\"title\"></li><li nav-link=\"Logout ({{ user.name }})\" url=\"/logout\"></li></ul><!-- staff --><ul layout=\"col sm-row\" ng-if=\"auth.isAuthenticated() && user.admin == 1\"><li class=\"title\">Bookings</li><li nav-link=\"My Bookings\" url=\"/\"></li><li nav-link=\"Make a Booking\" url=\"/book\"></li><li nav-link=\"All Bookings\" url=\"/bookings\"></li><li class=\"title\">Manage</li><li nav-link=\"Users\" url=\"/manage/user\" number=\"{{ store.users.length }}\"></li><li class=\"title\"></li><li nav-link=\"Logout ({{ user.name }})\" url=\"/logout\"></li></ul><!-- manager --><ul layout=\"col sm-row\" ng-if=\"auth.isAuthenticated() && user.admin == 2\"><li class=\"title\">Bookings</li><li nav-link=\"My Bookings\" url=\"/\"></li><li nav-link=\"Make a Booking\" url=\"/book\"></li><li nav-link=\"All Bookings\" url=\"/bookings\" number=\"{{ bookings_count }}\" loading=\"loading_bookings\"></li><li class=\"title\">Manage</li><li nav-link=\"Manage\" url=\"/manage\" show-only-sm></li><li nav-link=\"Users\" url=\"/manage/user\" number=\"{{ store.users.length }}\" loading=\"!store.users.$resolved\" class=\"sm-hide\"></li><li nav-link=\"Class Types\" url=\"/manage/class_type\" number=\"{{ store.group_types.length }}\" loading=\"!store.group_types.$resolved\" class=\"sm-hide\"></li><li nav-link=\"Classes\" url=\"/manage/class\" number=\"{{ store.groups.length }}\" loading=\"!store.groups.$resolved\" class=\"sm-hide\"></li><li nav-link=\"Product Types\" url=\"/manage/product_type\" number=\"{{ store.product_types.length }}\" loading=\"!store.product_types.$resolved\" class=\"sm-hide\"></li><li nav-link=\"Products\" url=\"/manage/product\" number=\"{{ store.products.length }}\" loading=\"!store.products.$resolved\" class=\"sm-hide\"></li><li nav-link=\"Units\" url=\"/manage/unit\" number=\"{{ store.units.length }}\" loading=\"!store.units.$resolved\" class=\"sm-hide\"></li><li class=\"title\"></li><li nav-link=\"Logout ({{ user.name }})\" url=\"/logout\"></li></ul></nav></div>");
