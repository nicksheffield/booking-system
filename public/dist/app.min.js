...
angular.module('app.controllers').controller('classProductCtrl', ["$scope", "$stateParams", "$store", "$load", "$location", "$http", "$timeout", "$invalidate", "sweetAlert", function ($scope, $stateParams, $store, $load, $location, $http, $timeout, $invalidate, sweetAlert) {
	...
	$scope.group.$promise.then(function (res) {
		console.log('res', res);
		$scope.selectedProducts = $scope.group.allowed_products.map(function (p) {
			return {
	...
	$scope.save = function () {
		$http.post('/api/group/' + $scope.group.id + '/products', $scope.selectedProducts).then(function (res) {
			console.log(res);
		});
	};
...
}]).controller('idleCtrl', ["$scope", "$location", "$http", "$store", "$auth", "Idle", function ($scope, $location, $http, $store, $auth, Idle) {
...
angular.module('app.services').factory('$invalidate', ["$rootScope", "$auth", "$store", "$interval", "$load", function ($rootScope, $auth, $store, $interval, $load) {
	...
	service.all = function () {
		if ($auth.getPayload() && $auth.getPayload().admin) {
			...
			service.add(['user', 'groups', 'products', 'units', 'settings', 'kits']);
		}
	};
