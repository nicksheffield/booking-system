...
angular.module('app.controllers').controller('makeCtrl', ["$scope", "$store", "$location", function ($scope, $store, $location) {
	...
	$scope.removeProduct = function (product) {
		...
		$scope.selected.products = _.reject($scope.selected.products, function (p) {
...
angular.module('app.controllers').controller('userImportCtrl', ["$scope", "$q", "$invalidate", "$store", "$location", "User", function ($scope, $q, $invalidate, $store, $location, User) {
...
angular.module('app.controllers').controller('userViewNoteCtrl', ["$scope", "$stateParams", "$store", "$invalidate", "$location", "Note", function ($scope, $stateParams, $store, $invalidate, $location, Note) {
	...
	$scope.deleteNote = function (note) {
		if (confirm('Are you sure you want to delete that note?')) {
			Note.delete({ id: note.id }).$promise.then(function () {
				$scope.user.notes = _.reject($scope.user.notes, function (n) {
					return n.id == note.id;
				});
			});
		}
...
angular.module("app.views").run(["$templateCache", function ($templateCache) {
  ...
  $templateCache.put("components/manage/users/views/import.html", "<page-header text=\"Import Users\"><btn type=\"primary\" icon=\"cloud-upload\" ng-click=\"import()\">Import</btn></page-header><div class=\"page-content\"><div class=\"importer\" layout=\"flex\" align=\"center\"><label>CSV File</label><input type=\"file\" class=\"form-control\" fileread=\"file\" loader=\"loader\" ratio=\"1\"><button class=\"btn btn-info\" ng-click=\"loader.load()\"><i class=\"fa fa-refresh\"></i></button></div><div class=\"error-box error-box\" ng-show=\"errors.length\"><div class=\"title\"><i class=\"fa fa-exclamation-triangle\"></i> Errors</div><div class=\"content\" layout=\"col\"><p ng-repeat=\"error in errors\" ratio=\"1\">{{ error.message }}</p></div></div><div ng-show=\"users.length\"><alert type=\"success\" icon=\"check\" text=\"Found {{ users.length }} Users\"></alert><table class=\"table\"><tr><th>name</th><th>email</th><th>id_number</th><th>phone</th><th>dob</th><th>password</th><th>group</th></tr><tr ng-repeat=\"user in users\"><td>{{ user.name }}</td><td>{{ user.email }}</td><td>{{ user.id_number }}</td><td>{{ user.phone }}</td><td>{{ user.dob }}</td><td>{{ user.password }}</td><td><span ng-show=\"user._group\">{{ user._group.code }}</span><span ng-show=\"user._nogroup\" class=\"pip pip-danger\">{{ user._nogroup }}</span></td></tr></table></div></div>");
