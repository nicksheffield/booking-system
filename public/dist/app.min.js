...
angular.module('app.controllers').controller('userImportCtrl', ["$scope", function ($scope) {}]);
'use strict';
...
angular.module('app.directives').directive('sidebar', ["$store", "$auth", "$load", "$rootScope", function ($store, $auth, $load, $rootScope) {
	function link(scope, el, attrs) {
		scope.auth = $auth;
		scope.user = $store.user;
...
angular.module("app.views").run(["$templateCache", function ($templateCache) {
  ...
  $templateCache.put("directives/breadcrumbs/breadcrumbs.html", "<div class=\"breadcrumbs\"><ul layout=\"row\"><li ng-repeat=\"crumb in crumbs\" layout=\"row\"><a href=\"{{ fix(crumb.url) }}\"> {{ crumb.data.title }}</a></li><li><a href=\"{{ current_url }}\">{{ state.current.data.title }}</a></li></ul></div>");
  ...
  $templateCache.put("directives/navLink/navLink.html", "<a href=\"{{ url }}\" layout=\"row\" align=\"center\"><i class=\"fa {{ \'fa-\' + icon }}\" ng-show=\"icon\"></i><span ng-show=\"!!navLink\">{{ navLink }}</span></a>");
  $templateCache.put("directives/pageHeader/pageHeader.html", "<header class=\"page-header\" layout=\"row\" align=\"center\" justify=\"between\"><breadcrumbs ng-show=\"showBreadcrumbs\"></breadcrumbs><h1>{{ text }}</h1><ng-transclude></ng-transclude></header>");
  $templateCache.put("directives/pager/pager.html", "<nav class=\"pagination-container\" layout=\"row\" justify=\"center\"><ul class=\"pagination\"><li class=\"page-item\" ng-class=\"{disabled: getCurrent() == 1}\"><span class=\"page-link\" ng-click=\"setCurrent(1)\">First</span></li><li class=\"page-item\" ng-class=\"{disabled: getCurrent() == 1}\"><span class=\"page-link\" ng-click=\"setCurrent(getCurrent() - 1)\">&laquo;</span></li><li class=\"page-item\" ng-repeat=\"page in pages\" ng-class=\"{active: getCurrent() == page}\"><span class=\"page-link\" ng-click=\"setCurrent(page)\">{{ page }}</span></li><li class=\"page-item\" ng-class=\"{disabled: getCurrent() == totalpages}\"><span class=\"page-link\" ng-click=\"setCurrent(getCurrent() + 1)\">&raquo;</span></li><li class=\"page-item\" ng-class=\"{disabled: getCurrent() == totalpages}\"><span class=\"page-link\" ng-click=\"setCurrent(totalpages)\">Last ({{ totalpages }})</span></li></ul><select ng-model=\"current\" ng-change=\"updateOld()\" ng-options=\"(\'page: \' + num) for num in allpages\" class=\"form-control page-picker\"></select></nav>");
  ...
  $templateCache.put("directives/sidebar/sidebar.html", "<div class=\"sidebar\" layout=\"row\" justify=\"between\"><div class=\"site-logo\" layout=\"row\" justify=\"center\" align=\"center\"><!-- <img src=\"/images/logo2.svg\" alt=\"\" class=\"logo\" height=\"50\"> --><div class=\"logo\"><img src=\"/images/Yoobee_Design_School_Logo_EPS.png\" height=\"50\" alt=\"\"><span class=\"caption\">Equipment Booking</span></div><!-- <h1>Booking System</h1> --></div><nav layout=\"row\"><!-- guest --><ul layout=\"row\" ng-if=\"!auth.isAuthenticated()\"><li nav-link=\"Login\" url=\"/login\"></li><!-- <li nav-link=\"Register\" url=\"/register\"></li> --></ul><!-- student --><ul layout=\"row\" ng-if=\"auth.isAuthenticated() && user.admin == 0\"><li nav-link=\"Home\" url=\"/\"></li><li nav-link=\"Make booking\" url=\"/book\"></li><li nav-link=\"Modify booking\" url=\"/book/edit\"></li><li nav-link=\"Logout\" url=\"/logout\"></li></ul><!-- staff --><ul layout=\"row\" ng-if=\"auth.isAuthenticated() && user.admin == 1\"><li nav-link=\"Home\" url=\"/\"></li><li nav-link=\"Book\" url=\"/book\"></li><li nav-link=\"Bookings\" url=\"/bookings\"></li><li nav-link=\"Users\" url=\"/manage/user\"></li><li nav-link=\"Logout\" url=\"/logout\"></li></ul><!-- manager --><ul layout=\"row\" ng-if=\"auth.isAuthenticated() && user.admin == 2\"><li nav-link=\"Home\" url=\"/\"></li><li nav-link=\"Book\" url=\"/book\"></li><li nav-link=\"Bookings\" url=\"/bookings\"></li><li nav-link=\"Manage\" url=\"/manage\"></li><li nav-link=\"Logout\" url=\"/logout\"></li></ul></nav></div>");
  ...
  $templateCache.put("components/booking/manage/manage.html", "<page-header text=\"All Bookings\"><div class=\"btn-group\"><button class=\"btn btn-primary btn-icon btn-sm\" ng-click=\"advFilter.toggleOpen()\"><i class=\"fa fa-filter\"></i>Filter</button><button type=\"button\" class=\"btn btn-primary btn-sm dropdown-toggle dropdown-toggle-split\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"><span class=\"sr-only\">Toggle Dropdown</span></button><div class=\"dropdown-menu dropdown-menu-right\"><span class=\"dropdown-item btn-icon\" ng-click=\"advFilter.clear()\"><i class=\"fa fa-undo\"></i>Reset</span></div></div></page-header><div class=\"page-advanced-filter\" ng-class=\"{\'filter-open\': advFilter.open}\" ng-show=\"advFilter.open\"><div class=\"filter-drawer\"><div layout=\"row\" justify=\"between\" class=\"pad-down\"><h2>Filter</h2><btn type=\"success\" icon=\"check\" ng-click=\"advFilter.apply()\" ng-show=\"advFilter.open\">Apply</btn></div><div layout=\"col\"><div layout=\"row\" ratio=\"1\"><label class=\"item checkbox-item\"><input type=\"checkbox\" ng-model=\"advFilter.props.overdue\"> Show overdue</label><label class=\"item checkbox-item\"><input type=\"checkbox\" ng-model=\"advFilter.props.closed\"> Show closed</label><label class=\"item checkbox-item\"><input type=\"checkbox\" ng-model=\"advFilter.props.issued\"> Show issued</label><label class=\"item checkbox-item\"><input type=\"checkbox\" ng-model=\"advFilter.props.booked\"> Show booked</label></div><div layout=\"row\" ratio=\"1\"><div class=\"item\"><label>After</label><div class=\"input-group\"><span class=\"input-group-addon\" id=\"basic-addon1\"><i class=\"fa fa-calendar\"></i></span><input type=\"date\" ng-model=\"advFilter.props.after\" class=\"form-control\"></div></div><div class=\"item\"><label>Before</label><div class=\"input-group\"><span class=\"input-group-addon\" id=\"basic-addon1\"><i class=\"fa fa-calendar\"></i></span><input type=\"date\" ng-model=\"advFilter.props.before\" class=\"form-control\"></div></div></div><div layout=\"row\"><div class=\"item has-select-clearer\" ratio=\"1\"><label>User</label><ui-select select-unit-on-close select-dark ng-model=\"advFilter.props.user\"><ui-select-match allow-clear=\"false\" placeholder=\"​\"><span ng-bind=\"$select.selected.name\" class=\"setheight\"></span></ui-select-match><ui-select-choices repeat=\"user in (users | filter: $select.search)\"><div ng-bind-html=\"user.name | highlight: $select.search\"></div><small ng-bind-html=\"user.group.code | highlight: $select.search\"></small></ui-select-choices></ui-select><button class=\"close\" ng-click=\"advFilter.clearUser()\" ng-show=\"advFilter.props.user\"><i class=\"fa fa-times-circle\"></i></button></div><div class=\"item\" ratio=\"1\"><label>Limit</label><!-- <input type=\"number\" ng-model=\"advFilter.props.limit\" class=\"form-control\"> --><div layout=\"row\"><input type=\"range\" class=\"form-control\" ng-model=\"advFilter.props.limit\" step=\"15\" min=\"10\" max=\"100\"><output>{{ advFilter.props.limit }}</output></div></div></div></div></div></div><div class=\"page-content\"><pager current=\"page\" limit=\"advFilter.props.limit\" data=\"bookings\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && bookings.length >= advFilter.props.limit && !loading\"></pager><div ng-show=\"bookings.length && !loading\"><a ng-repeat=\"booking in bookings | page: page : advFilter.props.limit : 30 as filtered\" href=\"booking/{{ booking.id }}\" class=\"booking filterable-row list-item\" layout=\"row sm-col\" align=\"center sm-stretch\"><div class=\"col-username filterable-cell\" ratio=\"1\"><div class=\"title\">User</div><div class=\"content\">{{ booking.user.name }}</div></div><div class=\"col-groupcode filterable-cell\" ratio=\"1\"><div class=\"title\">Class</div><div class=\"content\">{{ booking.user.group.code }}</div></div><div class=\"col-status status-{{ booking._status.toLowerCase() }}\" ratio=\"1\"><div class=\"title\">Status</div><div class=\"content\">{{ booking._status }}</div></div><div class=\"col-status2\" ratio=\"1\" ng-show=\"booking._overdue && booking._priority == 2\"><div class=\"title\">Late by</div><div class=\"content\" ng-class=\"{late: booking._overdue && booking._priority == 2}\">{{ booking._timeoverdue + (booking._timeoverdue == 1 ? \' day\' : \' days\') }}<span ng-show=\"booking._priority == 3\"> ago</span></div></div><div class=\"col-status2\" ratio=\"1\" ng-show=\"!booking._overdue && booking._priority == 2\"><div class=\"title\">Due at</div><div class=\"content\">{{ timediff(booking.due_at, \'days\') }} days</div></div><div class=\"col-status2\" ratio=\"1\" ng-show=\"booking._priority == 1\"><div class=\"title\">Pickup in</div><div class=\"content\">{{ timediff(booking.pickup_at, \'days\') }} days</div></div><div class=\"col-status2\" ratio=\"1\" ng-show=\"booking._priority == 3\"><div class=\"title\">Closed</div><div class=\"content\">{{ timediff(booking.closed_at, \'days\') }} days ago</div></div><!-- <div class=\"col-buttons button-row\" layout=\"row\" justify=\"end\" ratio=\"3\"><btn url=\"booking/{{ booking.id }}\" type=\"info\" icon=\"arrow-right\" small>View</btn></div> --></a></div><div ng-show=\"!bookings.length && !loading\"><alert type=\"danger\" icon=\"exclamation-triangle\" text=\"No bookings match your filter critera\"></alert></div><pager current=\"page\" limit=\"advFilter.props.limit\" data=\"bookings\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && bookings.length >= 30 && !loading\"></pager><div class=\"loading\" ng-show=\"loading\" layout=\"row\" justify=\"center\"><i class=\"fa fa-circle-o-notch fa-spin fa-3x fa-fw\"></i></div></div>");
  ...
  $templateCache.put("components/guest/login/login.html", "<header class=\"page-header\"><h1>Login</h1></header><div class=\"page-content\"><alert type=\"danger\" icon=\"exclamation-circle\" text=\"{{ error }}\" ng-show=\"error\"></alert><form ng-submit=\"login()\"><div class=\"form-group\"><label for=\"email\">Email</label><input type=\"text\" id=\"email\" ng-model=\"email\" class=\"form-control\"></div><div class=\"form-group\"><label for=\"password\">Password</label><input type=\"password\" id=\"password\" ng-model=\"password\" class=\"form-control\"></div><div class=\"form-group\"><input type=\"submit\" class=\"btn btn-primary\" value=\"Login\"></div></form></div>");
  ...
  $templateCache.put("components/manage/product_types/views/manage.html", "<page-header text=\"All Product Types\"><btn icon=\"plus\" url=\"manage/product_type/new\">New</btn></page-header><filter model=\"filter\" filtered=\"filtered\"></filter><div class=\"page-content\"><table class=\"table table-hover table-filtered\" ng-hide=\"!filtered.length\"><thead><tr><th class=\"table-col-name\">Name</th><th class=\"table-col-name\">No of Products</th><!-- <th class=\"table-col-controls\"></th> --></tr></thead><tbody><tr ng-repeat=\"type in product_types | filter: filter | page : page : 10 : 30 as filtered\"><td><a href=\"manage/product_type/{{ type.id }}\">{{ type.name }}</a></td><td>{{ type.products.length }}</td><!-- <td><btn url=\"manage/product_type/{{ type.id }}\" type=\"info\" icon=\"arrow-right\" small>View</btn></td> --></tr></tbody></table><pager current=\"page\" limit=\"10\" data=\"product_types\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && product_types.length >= 30\"></pager></div>");
  ...
  $templateCache.put("components/manage/users/views/import.html", "<page-header text=\"Import Users\"><btn type=\"primary\" icon=\"floppy-o\" ng-click=\"import()\">Import</btn></page-header><div class=\"page-content\"></div>");
