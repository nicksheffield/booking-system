	...
	function link(scope, el, attrs) {
		...
		scope.cols = scope.data.cols.map(function (col) {
			...
			col.sorter.switch = function () {
				return col.sorter.dir = !col.sorter.dir ? '+' : col.sorter.dir === '+' ? '-' : null;
			};
			col.filter = '';
...
angular.module('app.controllers').controller('userManageCtrl', ["$scope", "$store", "$location", function ($scope, $store, $location) {
	...
	$scope.dataTable = {
		...
		cols: [{
			...
			name: 'Class',
			prop: 'group.code', getter: function getter(x) {
...
angular.module("app.views").run(["$templateCache", function ($templateCache) {
  ...
  $templateCache.put("directives/data-table/data-table.html", "<div class=\"data-table\"><table class=\"table table-striped\"><thead><tr><th ng-click=\"sort()\" ng-repeat=\"col in cols track by $index\"><span class=\"sorter\" ng-class=\"{\'sorter-active\': col.sorter.dir}\" ng-click=\"col.sorter.switch()\">{{ col.name }} <span class=\"sorter-icon\"><i class=\"fa fa-sort\"      ng-if=\"!col.sorter.dir\"></i><i class=\"fa fa-sort-desc\" ng-if=\"col.sorter.dir == \'-\'\"></i><i class=\"fa fa-sort-asc\"  ng-if=\"col.sorter.dir == \'+\'\"></i></span></span><span ng-if=\"col.dropdown\"><dropdown ng-model=\"col.filter\" data=\"col.dropdown.items\" display=\"col.dropdown.display\" small nullable></dropdown></span><span ng-if=\"!col.dropdown\"><input type=\"text\" class=\"form-control form-control-sm\" ng-model=\"col.filter\" placeholder=\"filter...\"></span></th><th class=\"button-col\"></th></tr></thead><tr ng-repeat=\"item in items | filter: allFilters | orderBy: allSorts() | page: page : 10 : 30 as filtered track by $index\"><td ng-repeat=\"col in cols\">{{ col.getProp(item) }}</td><td class=\"button-col\"><a href=\"/manage/user/{{ item.id }}\" ng-if=\"buttons.view\"><i class=\"fa fa-fw fa-eye\"></i></a><a href=\"/manage/user/{{ item.id }}/edit\" ng-if=\"buttons.edit\"><i class=\"fa fa-fw fa-pencil\"></i></a><button ng-click=\"delete()\" class=\"btn-link\" ng-if=\"buttons.delete\"><i class=\"fa fa-fw fa-trash-o\"></i></button></td></tr></table></div>");
  ...
  $templateCache.put("pages/manage/users/manage/manage.html", "<page-header text=\"All Users\"><div class=\"btn-group\"><a href=\"manage/user/new\" class=\"btn btn-primary btn-icon btn-sm\"><i class=\"fa fa-plus\"></i>New User</a><button type=\"button\" class=\"btn btn-primary btn-sm dropdown-toggle dropdown-toggle-split\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"><span class=\"sr-only\">Toggle Dropdown</span></button><div class=\"dropdown-menu dropdown-menu-right\"><a class=\"dropdown-item btn-icon\" href=\"manage/user/import\"><i class=\"fa fa-cloud-upload\"></i>Import Users</a><a class=\"dropdown-item btn-icon\" href=\"manage/user/export\"><i class=\"fa fa-cloud-download\"></i>Export Users</a></div></div></page-header><div class=\"page-content\"><pager current=\"page\" limit=\"10\" data=\"users\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && users.length >= 30\"></pager><my-table data=\"dataTable\" page=\"page\" filtered=\"filtered\"></my-table></div><div class=\"page-content\" ng-hide=\"true\"><div ng-hide=\"!filtered.length\"><a class=\"list-item\" ng-repeat=\"user in users | filter: filter | orderBy: \'group.id\' | orderBy: \'-admin\' | page: page : 10 : 30 as filtered\" href=\"manage/user/{{ user.id }}\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Name</div><div class=\"content\">{{ user.name }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Role</div><div class=\"content\">{{ user._role }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\" ng-show=\"user.group\">Class</div><div class=\"content\">{{ user.group.code }}</div></div></a></div></div>");
