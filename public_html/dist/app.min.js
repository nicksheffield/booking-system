			...
			scope.cols.forEach(function (col) {
				if (col.filter.type === 'checkbox-dropdown') {
					if (col.filter.value) {
						...
						if (!col.filter.value.filter(function (x) {
							return x.checked && x[col.filter.display.text] === filterVal;
						}).length) {
							filtered = false;
						}
					}
				} else {
					var _filterVal = typeof col.filter.value === 'string' ? col.filter.value : col.filter.value !== null ? col.filter.value[col.filter.display.text] : '';

					if (col.getProp(value).toLowerCase().indexOf(_filterVal.toLowerCase()) === -1) {
						filtered = false;
					}
				}
			});
...
angular.module('app.controllers').controller('bookingsManageCtrl', ["$scope", "$store", "$load", "$stateParams", "$timeout", "$prepare", "Booking", function ($scope, $store, $load, $stateParams, $timeout, $prepare, Booking) {
	...
	$scope.dataTable = {
		...
		cols: [{
			...
			prop: 'created_at',
			getter: function getter(x) {
				return x.created_at ? moment(x.created_at).format('MMM Do') : '';
			},
			filter: {
				type: 'date'
			}
			...
			filter: {
				...
				items: [{ text: 'Closed', checked: false }, { text: 'Issued', checked: true }, { text: 'Booked', checked: true }],
				display: { text: 'text' }
			}
		}]
	};
