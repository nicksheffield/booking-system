...
angular.module('app.directives').directive('myTable', ["$filter", function ($filter) {
	function link(scope, el, attrs) {
		...
		scope.allFilters = function (value, index, array) {
			...
			scope.cols.forEach(function (col) {
				var filterVal = typeof col.filter === 'string' ? col.filter : col.filter !== null ? col.filter[col.dropdown.display.text] : '';
...
angular.module("app.views").run(["$templateCache", function ($templateCache) {
  ...
  $templateCache.put("directives/data-table/data-table.html", "<div class=\"data-table\"><div class=\"data-table-pagination\" layout=\"row\" justify=\"between\" align=\"center\"><label>Show <select ng-model=\"limit\" ng-options=\"value.n for value in limits\"></select> Entries</label><pagination page=\"page\" data=\"filtered\" limit=\"limit.n\"></pagination></div><table class=\"table table-striped\"><thead><tr><th class=\"button-col\"></th><th ng-click=\"sort()\" ng-repeat=\"col in cols track by $index\"><span class=\"sorter\" ng-class=\"{\'sorter-active\': col.sorter.dir}\" ng-click=\"col.sorter.switch()\" layout=\"row\" justify=\"between\">{{ col.name }} <span class=\"sorter-icon\"><i class=\"fa fa-sort\"      ng-if=\"!col.sorter.dir\"></i><i class=\"fa fa-sort-desc\" ng-if=\"col.sorter.dir == \'-\'\"></i><i class=\"fa fa-sort-asc\"  ng-if=\"col.sorter.dir == \'+\'\"></i></span></span></th></tr><tr><td></td><td ng-repeat=\"col in cols track by $index\"><span ng-if=\"col.dropdown\"><dropdown ng-model=\"col.filter\" data=\"col.dropdown.items\" display=\"col.dropdown.display\" small nullable></dropdown></span><span ng-if=\"!col.dropdown\"><input type=\"text\" class=\"form-control form-control-sm\" ng-model=\"col.filter\" placeholder=\"Filter...\"></span></td></tr></thead><tr ng-repeat=\"item in filtered | page: page : limit.n as paged\"><td class=\"button-col\"><a href=\"/manage/{{ data.slug }}/{{ item.id }}\" ng-if=\"buttons.view\"><i class=\"fa fa-fw fa-eye\"></i></a> <a href=\"/manage/{{ data.slug }}/{{ item.id }}/edit\" ng-if=\"buttons.edit\"><i class=\"fa fa-fw fa-pencil\"></i></a> <button ng-click=\"delete()\" class=\"btn-link\" ng-if=\"buttons.delete\"><i class=\"fa fa-fw fa-trash-o\"></i></button> </td><td ng-repeat=\"col in cols\">{{ col.getProp(item) }}</td></tr></table><div class=\"data-table-count\" layout=\"row\" justify=\"between\" align=\"center\"><span>Displaying {{ data.items.indexOf(paged[0]) + 1 }} - {{ data.items.indexOf(paged[paged.length - 1]) + 1 }} out of {{ filtered.length }} entries</span><pagination page=\"page\" data=\"filtered\" limit=\"limit.n\"></pagination></div></div>");
  ...
  $templateCache.put("pages/manage/classes/manage/manage.html", "<page-header text=\"All Classes\"><div class=\"button-row\"><btn icon=\"bars\" url=\"manage/class-type\" type=\"secondary\">Class Types</btn><btn icon=\"plus\" url=\"manage/class/new\">New Class Type</btn></div></page-header><!-- <filter model=\"filter\" filtered=\"filtered\"></filter> --><div class=\"page-content\"><my-table data=\"dataTable\"></my-table><!-- <a ng-repeat=\"group in groups | filter: filter | page : page : 10 as filtered\" href=\"manage/class/{{ group.id }}\" class=\"list-item\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Code</div><div class=\"content\">{{ group.code }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Type</div><div class=\"content\">{{ group.type.name }}</div></div></a><pager current=\"page\" limit=\"10\" data=\"groups\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && groups.length >= 30\"></pager> --></div>");
  ...
  $templateCache.put("pages/manage/kits/manage/manage.html", "<page-header text=\"All Kits\"><btn icon=\"plus\" url=\"manage/kit/new\">New Kit</btn></page-header><!-- <filter model=\"filter\" filtered=\"filtered\"></filter> --><div class=\"page-content\"><my-table data=\"dataTable\"></my-table><!-- <a ng-repeat=\"kit in kits | filter: filter | page : page : 10 as filtered\" href=\"manage/kit/{{ kit.id }}\" class=\"list-item\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Name</div><div class=\"content\">{{ kit.name }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">No. of Products</div><div class=\"content\">{{ kit.products.length }}</div></div></a> --><!-- <pager current=\"page\" limit=\"10\" data=\"kits\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && product_types.length >= 30\"></pager> --></div>");
  ...
  $templateCache.put("pages/manage/kits/view/view.html", "<page-header text=\"{{ kit.name }}\"><div class=\"btn-group\"><a href=\"manage/kit/{{ kit.id }}/edit\" class=\"btn btn-primary btn-sm btn-icon\"><i class=\"fa fa-pencil\"></i>Edit</a><button type=\"button\" class=\"btn btn-primary btn-sm dropdown-toggle dropdown-toggle-split\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"><span class=\"sr-only\">Toggle Dropdown</span></button><div class=\"dropdown-menu dropdown-menu-right\"><span class=\"dropdown-item btn-icon\" ng-click=\"delete()\"><i class=\"fa fa-trash\"></i>Delete</span></div></div></page-header><div class=\"page-content\"><table class=\"table info-table\"><tr><td class=\"row-property\">Name</td><td class=\"row-value\">{{ kit.name }}</td></tr><tr><td class=\"row-property\">Products ({{ kit.products.length }})</td><td class=\"row-value\"><ul><li ng-repeat=\"product in kit.products\"><a href=\"manage/product/{{ product.id }}\">{{ product.name }}</a> <strong>x{{ product.pivot.quantity }}</strong></li></ul></td></tr></table></div>");
  ...
  $templateCache.put("pages/manage/units/manage/manage.html", "<page-header text=\"All Units\"><div class=\"btn-group\"><a href=\"manage/unit/new\" class=\"btn btn-primary btn-icon btn-sm\"><i class=\"fa fa-plus\"></i>New Unit</a><button type=\"button\" class=\"btn btn-primary btn-sm dropdown-toggle dropdown-toggle-split\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"><span class=\"sr-only\">Toggle Dropdown</span></button><div class=\"dropdown-menu dropdown-menu-right\"><a class=\"dropdown-item btn-icon\" href=\"manage/unit/import\"><i class=\"fa fa-cloud-upload\"></i>Import Units</a><a class=\"dropdown-item btn-icon\" href=\"manage/unit/export\"><i class=\"fa fa-cloud-download\"></i>Export Units</a></div></div></page-header><!-- <filter model=\"filter\" filtered=\"filtered\"></filter> --><div class=\"page-content\"><my-table data=\"dataTable\"></my-table><!-- <a ng-repeat=\"unit in units | filter: filter | orderBy: [product_id, unit_number] | page: page : 10 as filtered\" href=\"manage/unit/{{ unit.id }}\" class=\"list-item\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Product</div><div class=\"content\">{{ unit.product.name }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Unit No.</div><div class=\"content\">{{ unit.unit_number }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Serial No.</div><div class=\"content\">{{ unit.serial_number }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Asset No.</div><div class=\"content\">{{ unit.asset_number }}</div></div></a> --><!-- <table class=\"table table-hover table-filtered\" ng-hide=\"!filtered.length\"><thead><tr><th class=\"table-col-name\" width=\"10%\">Unit No</th><th class=\"table-col-code\" width=\"10%\">Serial No</th><th class=\"table-col-name\" width=\"10%\">Asset No</th><th class=\"table-col-name\">Product</th></tr></thead><tbody><tr ng-repeat=\"unit in units | filter: filter | page: page : 10 as filtered\"><td><a href=\"manage/unit/{{ unit.id }}\">{{ unit.unit_number }}</a></td><td>{{ unit.serial_number }}</td><td>{{ unit.asset_number }}</td><td><a href=\"manage/product/{{ unit.product.id }}\">{{ unit.product.name }}</a></td></tr></tbody></table> --><!-- <pager current=\"page\" limit=\"10\" data=\"units\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && units.length >= 30\"></pager> --></div>");
