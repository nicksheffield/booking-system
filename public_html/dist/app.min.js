...
angular.module('app.directives').directive('myTable', ["$filter", function ($filter) {
	function link(scope, el, attrs) {
		...
		scope.allFilters = function (value, index, array) {
			var passFilter = true;
			...
			scope.cols.forEach(function (col) {
				...
				if (filter.type === 'dropdown2') {
					if (config.multiple) {
						...
						filterVal = filter.value.reduce(function (s, x) {
...
angular.module('app.controllers').controller('bookingsManageCtrl', ["$scope", "$store", "$load", "$stateParams", "$timeout", "$prepare", "Booking", function ($scope, $store, $load, $stateParams, $timeout, $prepare, Booking) {
	...
	$scope.statuses = [{ text: 'Closed' }, { text: 'Issued' }, { text: 'Booked' }];

	$scope.dropdown_test_data = [{ text: 'Option A', checked: false }, { text: 'Option 2', checked: true }, { text: 'Option III', checked: false }];
	...
	$scope.dataTable = {
		...
		cols: [{
			...
			prop: 'pickup_at',
			getter: function getter(x) {
				return x.pickup_at ? moment(x.pickup_at).format('MMM Do') : '';
			}
		}, {
			name: 'Issued',
			prop: 'issued_at',
			getter: function getter(x) {
				return x.taken_at ? moment(x.taken_at).format('MMM Do') : '';
			}
		}, {
			name: 'Due',
			prop: 'due_at',
			getter: function getter(x) {
				return x.due_at ? moment(x.due_at).format('MMM Do') : '';
			}
		}, {
			name: 'Closed',
			prop: 'closed_at',
			getter: function getter(x) {
				return x.closed_at ? moment(x.closed_at).format('MMM Do') : '';
			}
		}, {
			name: 'Status',
			prop: '_status',
...
angular.module("app.views").run(["$templateCache", function ($templateCache) {
  ...
  $templateCache.put("directives/data-table/data-table.html", "<div class=\"data-table\"><div class=\"data-table-pagination\" layout=\"row\" justify=\"between\" align=\"center\"><label>Show <select ng-model=\"data.limit\" ng-options=\"value for value in limits\"></select> Entries</label><pagination page=\"data.page\" data=\"filtered\" limit=\"data.limit\"></pagination></div><table class=\"table table-hover\"><thead><tr><th class=\"button-col\"></th><th ng-click=\"sort()\" ng-repeat=\"col in cols track by $index\"><span class=\"sorter\" ng-class=\"{\'sorter-active\': col.sorter.dir}\" ng-click=\"col.sorter.switch()\" layout=\"row\" justify=\"between\">{{ col.name }} <span class=\"sorter-icon\"><i class=\"fa fa-sort\"      ng-if=\"!col.sorter.dir\"></i><i class=\"fa fa-sort-desc\" ng-if=\"col.sorter.dir == \'-\'\"></i><i class=\"fa fa-sort-asc\"  ng-if=\"col.sorter.dir == \'+\'\"></i></span></span></th></tr><tr><td></td><td ng-repeat=\"col in cols track by $index\"><span ng-if=\"col.filter.type === \'dropdown\'\"><dropdown ng-model=\"col.filter.value\" data=\"col.filter.items\" display=\"col.filter.display\" small nullable></dropdown></span><span ng-if=\"col.filter.type === \'dropdown2\'\"><!-- <dropdown ng-model=\"col.filter.value\" data=\"col.filter.items\" display=\"col.filter.display\" small nullable></dropdown> --><dropdown2 ng-model=\"col.filter.value\" items=\"col.filter.items\" config=\"col.filter.config\"></dropdown2></span><span ng-if=\"col.filter.type === \'checkbox-dropdown\'\"><dropdown ng-model=\"col.filter.value\" data=\"col.filter.items\" display=\"col.filter.display\" small checkboxes></dropdown></span><span ng-if=\"col.filter.type === \'date\'\"><input type=\"date\" class=\"form-control form-control-sm\" ng-model=\"col.filter.value\" placeholder=\"Choose date...\"></span><span ng-if=\"col.filter.type === \'string\'\"><input type=\"text\" class=\"form-control form-control-sm\" ng-model=\"col.filter.value\" placeholder=\"Filter...\"></span></td></tr></thead><tr ng-repeat=\"item in filtered | page: data.page : data.limit as paged\"><td class=\"button-col\"><a href=\"/{{ data.slug }}/{{ item.id }}\" ng-if=\"buttons.view\"><i class=\"fa fa-fw fa-eye\"></i></a> <a href=\"/{{ data.slug }}/{{ item.id }}/edit\" ng-if=\"buttons.edit\"><i class=\"fa fa-fw fa-pencil\"></i></a> <button ng-click=\"delete()\" class=\"btn-link\" ng-if=\"buttons.delete\"><i class=\"fa fa-fw fa-trash-o\"></i></button> </td><td ng-repeat=\"col in cols\">{{ col.getProp(item) }}</td></tr><tr ng-if=\"!paged.length\" class=\"no-records\"><td colspan=\"{{ cols.length + 1}}\">No records to show</td></tr></table><div class=\"data-table-count\" layout=\"row\" justify=\"between\" align=\"center\"><span>Displaying {{ filtered.indexOf(paged[0]) + 1 }} - {{ filtered.indexOf(paged[paged.length - 1]) + 1 }} out of {{ filtered.length }} entries</span><pagination page=\"data.page\" data=\"filtered\" limit=\"data.limit\"></pagination></div></div>");
  ...
  $templateCache.put("pages/manage/class-types/manage/manage.html", "<page-header text=\"All Class Types\"><div class=\"button-row\"><btn icon=\"bars\" url=\"/class\" type=\"secondary\">Classes</btn><btn icon=\"plus\" url=\"/class-type/new\">New Class</btn></div></page-header><!-- <filter model=\"filter\" filtered=\"filtered\"></filter> --><div class=\"page-content\"><my-table data=\"dataTable\"></my-table><!-- <a ng-repeat=\"type in group_types | filter: filter | page : 10 : 30 as filtered\" href=\"/class-type/{{ type.id }}\" class=\"list-item\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Name</div><div class=\"content\">{{ type.name }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Code</div><div class=\"content\">{{ type.code }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">No. of Classes</div><div class=\"content\">{{ type.groups.length }}</div></div></a><pager current=\"page\" limit=\"10\" data=\"group_types\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && group_types.length >= 30\"></pager> --></div>");
  ...
  $templateCache.put("pages/manage/classes/manage/manage.html", "<page-header text=\"All Classes\"><div class=\"button-row\"><btn icon=\"bars\" url=\"/class-type\" type=\"secondary\">Class Types</btn><btn icon=\"plus\" url=\"/class/new\">New Class Type</btn></div></page-header><!-- <filter model=\"filter\" filtered=\"filtered\"></filter> --><div class=\"page-content\"><my-table data=\"dataTable\"></my-table><!-- <a ng-repeat=\"group in groups | filter: filter | page : page : 10 as filtered\" href=\"/class/{{ group.id }}\" class=\"list-item\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Code</div><div class=\"content\">{{ group.code }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Type</div><div class=\"content\">{{ group.type.name }}</div></div></a><pager current=\"page\" limit=\"10\" data=\"groups\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && groups.length >= 30\"></pager> --></div>");
  ...
  $templateCache.put("pages/manage/kits/manage/manage.html", "<page-header text=\"All Kits\"><btn icon=\"plus\" url=\"/kit/new\">New Kit</btn></page-header><!-- <filter model=\"filter\" filtered=\"filtered\"></filter> --><div class=\"page-content\"><my-table data=\"dataTable\"></my-table><!-- <a ng-repeat=\"kit in kits | filter: filter | page : page : 10 as filtered\" href=\"/kit/{{ kit.id }}\" class=\"list-item\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Name</div><div class=\"content\">{{ kit.name }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">No. of Products</div><div class=\"content\">{{ kit.products.length }}</div></div></a> --><!-- <pager current=\"page\" limit=\"10\" data=\"kits\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && product_types.length >= 30\"></pager> --></div>");
  ...
  $templateCache.put("pages/manage/product-types/manage/manage.html", "<page-header text=\"All Product Types\"><div class=\"button-row\"><btn icon=\"bars\" url=\"/product\" type=\"secondary\">Products</btn><btn icon=\"plus\" url=\"/product-type/new\">New Product Type</btn></div></page-header><!-- <filter model=\"filter\" filtered=\"filtered\"></filter> --><div class=\"page-content\"><my-table data=\"dataTable\"></my-table><!-- <a ng-repeat=\"type in product_types | filter: filter | page : page : 10 : 30 as filtered\" href=\"/product-type/{{ type.id }}\" class=\"list-item\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Name</div><div class=\"content\">{{ type.name }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">No. of Products</div><div class=\"content\">{{ type.products.length }}</div></div></a><pager current=\"page\" limit=\"10\" data=\"product_types\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && product_types.length >= 30\"></pager> --></div>");
  ...
  $templateCache.put("pages/manage/products/manage/manage.html", "<page-header text=\"All Products\"><div class=\"button-row\"><btn icon=\"bars\" url=\"/product-type\" type=\"secondary\">Product Types</btn><btn icon=\"plus\" url=\"/product/new\">New Product</btn></div></page-header><!-- <filter model=\"filter\" filtered=\"filtered\"></filter> --><div class=\"page-content\"><my-table data=\"dataTable\"></my-table><!-- <a ng-repeat=\"product in products | filter: filter | page : page : 10 : 30 as filtered\" href=\"/product/{{ product.id }}\" class=\"list-item\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Name</div><div class=\"content\">{{ product.name }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Type</div><div class=\"content\">{{ product.type.name }}</div></div></a><pager current=\"page\" limit=\"10\" data=\"products\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && products.length >= 30\"></pager> --></div>");
  ...
  $templateCache.put("pages/manage/products/view/view.html", "<page-header text=\"{{ product.name }}\"><div class=\"btn-group\"><a href=\"/product/{{ product.id }}/edit\" class=\"btn btn-primary btn-sm btn-icon\"><i class=\"fa fa-pencil\"></i>Edit</a><button type=\"button\" class=\"btn btn-primary btn-sm dropdown-toggle dropdown-toggle-split\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"><span class=\"sr-only\">Toggle Dropdown</span></button><div class=\"dropdown-menu dropdown-menu-right\"><span class=\"dropdown-item btn-icon\" ng-click=\"delete()\"><i class=\"fa fa-trash\"></i>Delete</span></div></div></page-header><div class=\"page-content\"><table class=\"table info-table\"><tr><td class=\"row-property\">Name</td><td class=\"row-value\">{{ product.name }}</td></tr><tr><td class=\"row-property\">Type</td><td class=\"row-value\"><a href=\"/product-type/{{ product.type.id }}\">{{ product.type.name }}</a></td></tr><tr><td class=\"row-property\">Limitless</td><td class=\"row-value\">{{ product.limitless ? \'Yes\' : \'No\' }}</td></tr><tr><td class=\"row-property\">Classe Types ({{ product.group_types.length }})</td><td class=\"row-value\"><div layout=\"row\" align=\"start\" justify=\"between\" ng-show=\"product.group_types.length\"><ul><li ng-repeat=\"group_type in product.group_types\"><a href=\"/class-type/{{ group_type.id }}\">{{ group_type.name }}</a></li></ul></div><alert type=\"info\" text=\"No classe types are allowed to book this product. Edit a class type to allow this product.\" icon=\"exclamation-circle\" ng-if=\"!product.group_types.length\"></alert></td></tr><tr><td class=\"row-property\">Kits ({{ product.kits.length }})</td><td class=\"row-value\"><ul><li ng-repeat=\"kit in product.kits track by $index\"><a href=\"/kit/{{ kit.id }}\">{{kit.name }}</a></li></ul><alert type=\"info\" text=\"No kits for this product\" icon=\"exclamation-circle\" ng-if=\"!product.kits.length\"></alert></td></tr><tr ng-show=\"!product.limitless\"><td class=\"row-property\">Units ({{ product.units.length }})<br><a href=\"/unit/new?product={{ product.id }}\" class=\"btn push-down btn-primary btn-sm\"><i class=\"fa fa-plus\"></i></a></td><td class=\"row-value\"><ul><li ng-repeat=\"unit in product.units | orderBy: orderByUnit track by $index\"><a href=\"/unit/{{ unit.id }}\">{{ unit.unit_number }}</a></li></ul><alert type=\"info\" text=\"No units for this product\" icon=\"exclamation-circle\" ng-if=\"!product.units.length\"><btn url=\"/unit/new?product={{ product.id }}\" small icon=\"plus\">Create one</btn></alert></td></tr></table></div>");
  ...
  $templateCache.put("pages/manage/units/manage/manage.html", "<page-header text=\"All Units\"><div class=\"btn-group\"><a href=\"/unit/new\" class=\"btn btn-primary btn-icon btn-sm\"><i class=\"fa fa-plus\"></i>New Unit</a><button type=\"button\" class=\"btn btn-primary btn-sm dropdown-toggle dropdown-toggle-split\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"><span class=\"sr-only\">Toggle Dropdown</span></button><div class=\"dropdown-menu dropdown-menu-right\"><a class=\"dropdown-item btn-icon\" href=\"/unit/import\"><i class=\"fa fa-cloud-upload\"></i>Import Units</a><a class=\"dropdown-item btn-icon\" href=\"/unit/export\"><i class=\"fa fa-cloud-download\"></i>Export Units</a></div></div></page-header><!-- <filter model=\"filter\" filtered=\"filtered\"></filter> --><div class=\"page-content\"><my-table data=\"dataTable\"></my-table><!-- <a ng-repeat=\"unit in units | filter: filter | orderBy: [product_id, unit_number] | page: page : 10 as filtered\" href=\"/unit/{{ unit.id }}\" class=\"list-item\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Product</div><div class=\"content\">{{ unit.product.name }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Unit No.</div><div class=\"content\">{{ unit.unit_number }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Serial No.</div><div class=\"content\">{{ unit.serial_number }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Asset No.</div><div class=\"content\">{{ unit.asset_number }}</div></div></a> --><!-- <table class=\"table table-hover table-filtered\" ng-hide=\"!filtered.length\"><thead><tr><th class=\"table-col-name\" width=\"10%\">Unit No</th><th class=\"table-col-code\" width=\"10%\">Serial No</th><th class=\"table-col-name\" width=\"10%\">Asset No</th><th class=\"table-col-name\">Product</th></tr></thead><tbody><tr ng-repeat=\"unit in units | filter: filter | page: page : 10 as filtered\"><td><a href=\"/unit/{{ unit.id }}\">{{ unit.unit_number }}</a></td><td>{{ unit.serial_number }}</td><td>{{ unit.asset_number }}</td><td><a href=\"/product/{{ unit.product.id }}\">{{ unit.product.name }}</a></td></tr></tbody></table> --><!-- <pager current=\"page\" limit=\"10\" data=\"units\" filtered=\"filtered\" filter=\"filter\" ng-show=\"!filter && units.length >= 30\"></pager> --></div>");
  ...
  $templateCache.put("pages/manage/users/manage/manage.html", "<page-header text=\"All Users\"><div class=\"btn-group\"><a href=\"/user/new\" class=\"btn btn-primary btn-icon btn-sm\"><i class=\"fa fa-plus\"></i>New User</a><button type=\"button\" class=\"btn btn-primary btn-sm dropdown-toggle dropdown-toggle-split\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"><span class=\"sr-only\">Toggle Dropdown</span></button><div class=\"dropdown-menu dropdown-menu-right\"><a class=\"dropdown-item btn-icon\" href=\"/user/import\"><i class=\"fa fa-cloud-upload\"></i>Import Users</a><a class=\"dropdown-item btn-icon\" href=\"/user/export\"><i class=\"fa fa-cloud-download\"></i>Export Users</a></div></div></page-header><div class=\"page-content\"><my-table data=\"dataTable\"></my-table></div><div class=\"page-content\" ng-hide=\"true\"><div ng-hide=\"!filtered.length\"><a class=\"list-item\" ng-repeat=\"user in users | filter: filter | orderBy: \'group.id\' | orderBy: \'-admin\' | page: page : 10 as filtered\" href=\"/user/{{ user.id }}\"><div class=\"col\" ratio=\"1\"><div class=\"title\">Name</div><div class=\"content\">{{ user.name }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\">Role</div><div class=\"content\">{{ user._role }}</div></div><div class=\"col\" ratio=\"1\"><div class=\"title\" ng-show=\"user.group\">Class</div><div class=\"content\">{{ user.group.code }}</div></div></a></div></div>");
